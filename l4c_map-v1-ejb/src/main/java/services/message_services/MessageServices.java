...
public class MessageServices implements MessageLocal, MessageRemote {
		...
		if (projects != null) {
			projects.forEach(p -> {
				List<Message> messages = em
						.createQuery("select m from Message m where m.project.id = :projectId", Message.class)
						.setParameter("projectId", p.getIdProject()).getResultList();
				if (messages != null)
	...
	public int addMessage(Message message) {
		// if the client made a bad status to the ressource we gonna change his
		// score
		if (message.getFromto().equals("client->ressource")) { // message.reciver
																// is the the id
																// of the entity
																// who's the
																// message is
		...
		// if the message is directed to the whole project team ,they will all
		// suffer of the bad score
		else if (message.getFromto().equals("client->project")) {
			message.getProject().getListemandate().forEach(m -> {
